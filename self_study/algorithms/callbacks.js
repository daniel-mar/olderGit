...
function dropIt(arr, cb) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if(cb(arr[i]) === false) {
      let temp = arr.shift(arr[i]);
      // array shifts down when we remove on false
      // decrement to check the updated array index
      i--
      console.log(`was removed ${temp}`)
    }
  }
  return arr
}
...
function canStringBecomePalindrome(str) {
  ...
  let occurences = {};
  for (let char of str) {
    occurences[char] ? (occurences[char] += 1) : (occurences[char] = 1);
  }
  let oddCount = 0;
  for (let o in occurences) {
    occurences[o] % 2 !== 0 && oddCount++;
  }
